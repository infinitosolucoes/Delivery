...
class ListaPrecoController extends Controller
{
	...
	public function save(Request $request){
		...
		if($result){
			...
			session()->flash("message", "Lista cadastrada com sucesso.");
		}else{
			session()->flash('color', 'red');
			session()->flash('message', 'Erro ao cadastrar lista.');
		}
	...
	public function update(Request $request){
		...
		if($lista->save()){
			session()->flash('color', 'blue');
			session()->flash("message", "Lista atualizada com sucesso.");
		}else{
			session()->flash('color', 'red');
			session()->flash('message', 'Erro ao atualizar lista.');
		}
		return redirect('/listaDePrecos');
	}
	...
	public function gerar($id){
		...
		session()->flash("message", "Produtos cadastrados na lista $lista->nome");
	...
	public function salvarPreco(Request $request){
		...
		session()->flash('color', 'blue');
		session()->flash("message", "Valor atualizado do produto " . $produto->produto->nome);
	...
	public function delete($id){
		...
		session()->flash('color', 'green');
		session()->flash("message", "Lista removida!");
		return redirect('/listaDePrecos');
	}
