		...
		<div class="col s12">
			...
			<table class="pure-table">
				...
				<tbody>
					...
					@if(sizeof($i->produto->composto))
						@foreach($i->produto->receita->itens as $key2 => $it)
							<tr >
								<td><label style="margin-left: 20px;">{{$key+1}}.{{$key2+1}}. {{$i->produto->nome}}</label></td>
								<td><label>{{number_format($it->quantidade, 2)}}</label></td>
								<td><label>{{number_format($it->produto->valor_venda, 2)}}</label></td>
								<td><label>{{number_format($it->quantidade * $it->produto->valor_venda, 2)}}</label></td>
